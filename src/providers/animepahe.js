...
export class AnimePahe {
  ...
  async _recursiveFetchEpisodes(url, responseData, session) {
    try {
      ...
      if (hasNextPage) {
        const baseUrlPart = url.includes("&page=") ? url.split("&page=")[0] : url;
        const newUrl = `${baseUrlPart}&page=${page + 1}`;
        ...
        // Fetch details from the main anime page on the last recursive call
        ...
        if (newResponse.status === 200) {
          ...
          try {
            ...
            if (typeText) animeDetails.type = typeText.replace('Type:', '').trim();

            const statusText = $('.col-sm-4.anime-info p:contains("Status")').text();
            if (statusText) animeDetails.status = statusText.replace('Status:', '').trim();
            ...
            if (scoreText) {
              ...
              if (scoreMatch) animeDetails.score = parseFloat(scoreMatch[1]);
            }
          } catch (err) {
  ...
  /**
   * THE FIX: Scrape Kwik links and extract direct video URLs
   */
  async scrapeEpisodesSrcs(episodeId) {
    try {
      ...
      for (let i = 0; i < buttons.length; i++) {
        ...
        try {
          // Pass the kwikLink to our fixed extractor
      ...
      return {
        // CRITICAL: The consumer (player) must use these headers to avoid 403
        headers: {
  ...
  /**
   * Helper to find the correct session ID for an anime
   */
  async _getSession(title, animeId) {
    try {
      ...
      if (animeId) {
        ...
        if (animeIdMatch) return animeIdMatch.session;
      }
      ...
      return (bestMatch && highestSimilarity > 0.5) ? bestMatch.session : resBody.data[0].session;
    } catch (error) {
